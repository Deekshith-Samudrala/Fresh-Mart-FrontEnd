...
const Checkout = () => {
    ...
    let handlepayment = async()=>{
        ...
        let totalcartvalue = (coupondiscount > 0) ? (totalprice - totaldiscount + shipping)-((totalprice - totaldiscount + shipping)*coupondiscount/100) : totalprice - totaldiscount + shipping;
        
        let data = {
            totalprice : totalcartvalue
        }

        let result = await Paymentservice.placeorder(data);
        ...
        if(result.success){
            const options = {
                ...
                name : userdata.name,
                description : result.data.created_at,
                image : `${assetspath}/img/freshmartlogo.png`,
                order_id : result.data.id,
                handler : async(response)=>{
                    try{
                        let result = await Paymentservice.verifyorder(response);
                        if(result.success){
                            disp(removeallitems());
                            // yet to add the into the orders of the user 
                            navigate("/");
                        }
                        else{
                            alert("payment is not verified, Your order is on Hold !")
                        }
                    }
                    catch(error){
                        console.log(error);
            ...
            <div className='row mb-5'>
                ...
                <div className='col-md-4'>
                    ...
                    <div className='border-top'>
                        <button className='btn btn-primary mr-4 mt-4' onClick={handlepayment}><h4 className='text-light'>Place Order</h4></button>
