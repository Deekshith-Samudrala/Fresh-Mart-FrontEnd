...
let userdeletecartremove = createAsyncThunk("removecartonuserdeletion", async(userid)=>{
    
    let result = await axios.delete(`${api}/cart/delete/${userid}`);
    return result.data; 
})
...
let removeoneitem = createAsyncThunk("removeoneitemfromcart",async(unqid,{getState})=>{
    ...
    let result = await axios.delete(`${api}/cart/${userid}/item/${unqid}`);
    return result.data;
})
...
let Cartslice = createSlice({
    ...
    extraReducers : {
        ...
        [userdeletecartremove.fulfilled] : (state,action)=>{
            return ;
        }
    }
})
...
export { cartadd ,cartinit ,getcartitems, removeallitems, removeoneitem, emptycartonlogout, userdeletecartremove};